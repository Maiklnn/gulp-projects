class Run {
	...
	events() {
		...
		document.addEventListener('click', e => {
			...
			if(this.target.closest('[data-burger]')) this.mobileMenu()
		});
	}

	mobileMenu() {
		let burger = this.target.closest('[data-burger]')
		console.log(burger);
	...
	sliders() {
		var nameSlider = 'home-page-slider-top';
		new Swiper(document.querySelector(`.${nameSlider}__box`), {
			spaceBetween: 30,
			// растояние
		...
		// map-slider
		const sliderNav = document.querySelector('.page-map-slider-nav');
		...
		var nameSlider = 'page-map-slider-main';
...
document.addEventListener('click', e => {
	let target = e.target
	let clickedElement = target.closest('[data-show]')
	if(clickedElement)
	{
		e.preventDefault()
		let data = target.dataset.show.split(':')
...
// класс закрытия
class Close extends Main {
	constructor(e) {
		super();
		this.element = e.element;
		this.close_window = document.querySelector(`[data-target="${this.element.dataset.close}"]`);
		this.modalContainer = this.close_window.querySelector(`.modal-popup__container`);

		this.modalContainer.classList.remove('animate-open');
		this.close_window.classList.remove('is-open');

		this.enableScroll();

		if(e.settings.indexOf( 'dom') === -1) {
			setTimeout(() => { this.close_window.remove(); }, 5);
		}
...
class Load {
	constructor(options) {
		...
		this.load_item = this.element.querySelector('[data-load-item]');
		this.container.innerHTML = this.load_item.outerHTML;
	}
}

// создание экземпляра модалки
new Modal({});
...
class Form extends Main{
	...
	async submitForm() {
		...
		let success_modal = this.form.querySelector('.modal-popup')
		success_modal.classList.add('is-open')
		let data = await this.sendRequest();
		this.form.reset();
	}
